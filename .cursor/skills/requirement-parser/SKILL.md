...
2. **Prefer** persisted tooling: run `.forge/scripts/normalize_requirements_to_tmp.py --version vX` via `.venv` when it exists (see `rules/forge-scripts.mdc`). If that script is missing or insufficient, **add or extend** a script under `/.forge/scripts/` and document it, then run it — do not rely on throwaway one-liners as the only artifact.
...
* prefer `/.forge/scripts/` for repeatable normalize/changelog steps; extend there if missing (`rules/forge-scripts.mdc`)
