...
2. Build only in declared scope; put **application** installs, app venvs, and product build trees under **`projects/`** (see **`.cursor/rules/sandbox-projects.mdc`**, **`sandbox-execution`** skill) — root **`.venv`** is for Forge scripts only
3. Run tests and validation: use root **`.venv`** only for Forge scripts/tooling (`.cursor/rules/python-runtime.mdc`); for implementation under **`projects/`**, use that subtree’s toolchain and test commands
